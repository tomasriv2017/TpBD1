...
create procedure continuarMontaje(in _chasis int , out chasisOcupado int ,out cMensaje varchar(100), out nResultado int)
 BEGIN
    ...
    DECLARE curEstacion CURSOR FOR SELECT idEstacionTrabajo from estaciontrabajo where idEstacionTrabajo = 
					(SELECT EstacionTrabajo_idEstacionTrabajo FROM automovil_estaciontrabajo  where Automovil_chasis = _chasis) 
							and lineaMontaje = (select idLineaMontajeAsignada from automovil where chasis = _chasis);
    OPEN curEstacion;
    ...
    SELECT EstacionTrabajo_idEstacionTrabajo into idEstacionTrabajoActual FROM automovil_estaciontrabajo  where Automovil_chasis = _chasis;
    ...
    ELSE
		...
		SELECT count(*) into cantidadRepetida from automovil_estaciontrabajo where EstacionTrabajo_idEstacionTrabajo = idEstacionTrabajoSiguente;
		...
		if(cantidadRepetida > 0) then
			...
			Select Automovil_chasis into chasisOcupado from automovil_estaciontrabajo
								where EstacionTrabajo_idEstacionTrabajo = idEstacionTrabajoSiguente ;
		else        
