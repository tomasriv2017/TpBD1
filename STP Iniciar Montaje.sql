...
BEGIN
	...
	SELECT count(*) into cantidadRepetida from automovil_estaciontrabajo where EstacionTrabajo_idEstacionTrabajo = 
					(select min(idEstacionTrabajo) from estaciontrabajo  where lineamontaje = (select idLineaMontajeAsignada from automovil where chasis = _chasis));
