...
/*BAJA*/
DELIMITER $$
CREATE PROCEDURE eliminarPedido( in _idPedido int ,  in _idModeloVehiculo int , out cMensaje varchar(100) , out nResultado int )
BEGIN
	declare cantidadRepetida int default 0;
    ...
    if(cantidadRepetida > 0) then
		select 0 into nResultado;
        delete from detalle_pedido where ModeloVehiculo_idModeloVehiculo = _idModeloVehiculo and Pedido_idPedido = _idPedido;
...
/*MODIFICACION*/
DELIMITER $$
CREATE PROCEDURE modificarPedido( in _idPedido int, in _idModeloVehiculo int , in nueva_cantidadPedido int , in nueva_fechaPedido datetime , in nuevo_cuitConcensionaria varchar(45) , out cMensaje varchar(100) , out nResultado int )
BEGIN
	declare cantidadRepetida int default 0;
    select count(*) into cantidadRepetida from detalle_pedido where ModeloVehiculo_idModeloVehiculo = _idModeloVehiculo and
																		Pedido_idPedido = _idPedido;
    if(cantidadRepetida > 0) then
		if(nueva_cantidadPedido <> (select cantPedido from detalle_pedido where ModeloVehiculo_idModeloVehiculo = _idModeloVehiculo and Pedido_idPedido = _idPedido) 
            ...
            update detalle_pedido set cantPedido = nueva_cantidadPedido where ModeloVehiculo_idModeloVehiculo = _idModeloVehiculo and Pedido_idPedido = _idPedido;
